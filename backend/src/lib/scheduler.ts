...
export class AlertScheduler {
  ...
  start(): void {
    ...
    try {
      ...
      console.log('✅ Scheduler started with cron pattern:', appConfig.POLL_CRON);
      console.log('⏰ Next run scheduled for:', this.stats.nextRunTime);
    } catch (error) {
  ...
  private async executePoll(): Promise<void> {
    ...
    try {
      ...
      // Process global feeds if enabled
      if (appConfig.ENABLE_BEST_DEALS) {
        try {
          const bestDealsAlerts = await this.processBestDeals();
          newAlerts += bestDealsAlerts;
        } catch (error) {
          console.error('❌ Error processing best deals:', error);
          this.stats.errorCount++;
        }
      }

      if (appConfig.ENABLE_NEWEST_ITEMS) {
        try {
          const newestAlerts = await this.processNewestItems();
          newAlerts += newestAlerts;
        } catch (error) {
          console.error('❌ Error processing newest items:', error);
          this.stats.errorCount++;
        }
      }
  ...
  /**
   * Process best deals feed
   */
  private async processBestDeals(): Promise<number> {
    console.log('💎 Processing best deals feed...');

    try {
      const response = await this.skinBaronClient.getBestDeals({
        limit: 20,
        maxPrice: appConfig.FEEDS_MAX_PRICE,
        maxWear: appConfig.FEEDS_MAX_WEAR,
      });

      if (!response.success || !response.items) {
        console.log('⚠️  No best deals found');
        return 0;
      }
      ...
      for (const item of response.items) {
        ...
        try {
          ...
          if (success) {
            newAlerts++;
            console.log(`💎 Best deal alert: ${item.itemName} (${item.price} EUR)`);
          }
      ...
      console.error('❌ Failed to process best deals:', error);
      throw error;
    }
  }

  /**
   * Process newest items feed
   */
  private async processNewestItems(): Promise<number> {
    console.log('🆕 Processing newest items feed...');

    try {
      const response = await this.skinBaronClient.getNewestItems({
        limit: 20,
        maxPrice: appConfig.FEEDS_MAX_PRICE,
        maxWear: appConfig.FEEDS_MAX_WEAR,
      });
      ...
      if (!response.success || !response.items) {
        console.log('⚠️  No newest items found');
        return 0;
      }
      ...
      for (const item of response.items) {
        ...
        try {
          ...
          if (success) {
            newAlerts++;
            console.log(`🆕 New item alert: ${item.itemName} (${item.price} EUR)`);
          }
      ...
      console.error('❌ Failed to process newest items:', error);
      throw error;
    }
  }
